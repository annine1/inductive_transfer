...
from neuralhydrology.augmentation.scaling import Scaling
from neuralhydrology.augmentation.mbb import MBB
from neuralhydrology.augmentation.taskaug import Taskaug
...
class BaseTrainer(object):
    ...
    def __init__(self, cfg: Config):
        ...
        # self.basins = load_basin_file(cfg.train_basin_file, cfg.train_basin_file1)
        self.cfg.number_of_basins = len(self.basins)
        
        # self.basins1 = load_basin_file(cfg.train_basin_file1)
        # self.cfg.number_of_basins1 = len(self.basins1)
    ...
    # def _get_augment(self):# -> augmentation:
        # return get_augment(cfg=self.cfg)
    ...
    def _train_epoch(self, epoch: int):
        ...
        noise_aug = Noise(self.cfg.mu, self.cfg.std)
        cmixup_aug = CMixup(self.cfg.alpha, self.cfg.bw)
        scaling = Scaling(self.cfg.mu1, self.cfg.std1)
        movingbb = MBB(self.cfg.block_size, self.cfg.seed, self.cfg.n_Bootstrap)
        addtemp = Taskaug()
        
        # keys_listX = ["y", "date"]
        # keys_listy = ["x_d", "x_s", "date"]
