    ...
    def forward(self, data: Dict[str, torch.Tensor], concatenate_output: bool = True) \
        ...
        if not concatenate_output:
            ...
            if statics_out is not None:
                ...
                # # statics_out = statics_out.reshape(-1, 20, 28)
                # ret_val = torch.cat([dynamics_out, statics_out], dim=2)
                # # ###########################
                
                ### Repeat Elements (if you want to increase the number of samples by repeating the existing ones)
                # statics_out = statics_out.repeat(2, 1)[:384]
                statics_out = statics_out.unsqueeze(0).repeat(dynamics_out.shape[0], 1, 1)
