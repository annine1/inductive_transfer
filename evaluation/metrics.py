...
def get_available_metrics() -> List[str]:
    ...
    metrics = [
        ...
        "Peak-Timing", "Missed-Peaks", "Peak-MAPE", "PBias", "Bias"
    ]
    return metrics
...
def pbias(obs: DataArray, sim:DataArray) -> float:
    r""" PBias (percent bias) quantifies the average tendency of the simulated values to be larger or smaller than 
    their observed counterparts.
    .. math:: \text{PBias} = (\frac{\sum{i=1}^{n}(Sim_i - Obs_i)}{\sum{i=1}^{n}(Obs_i)})*100
    PBias=(sum(Sim - Obs)/sum(Obs))*100
    PBias of 0 indicates a perfect match between sim and obs values. 
    Positive PBias values indicate a model tendency to overestimate, while negative values indicate a tendency of
    underestimate. 
    """
    # verify inputs
    _validate_inputs(obs, sim)
    ...
    # # calculate the percent bias error
    # value = (np.sum(sim - obs) / np.sum(obs)) * 100
    
    return float((np.sum(sim - obs) / np.sum(obs)) * 100)

def bias(obs: DataArray, sim:DataArray) -> float:
    r"""
    Bias Error, also known as Mean Bias Error (MBE), is a metric used to assess the accuracy of a forecast model
    by measuring the average tendency of the model to overestimate or underestimate the actual values. 
    It is particularly useful in model validation and calibration across various fields such as weather forecasting, finance, and engineering.

The formula for calculating Bias Error (Mean Bias Error) is:

Bias = \frac{1}{n}\sum{t=1}^{n}(sim_t - obs_t)
    """
    # verify inputs
    _validate_inputs(obs, sim)
...
def calculate_metrics(obs: DataArray,
    ...
    for metric in metrics:
        if metric.lower() == "nse":
            ...
            values["PBias"] = pbias(obs, sim)
        elif metric.lower() == "bias":
            values["Bias"] = bias(obs, sim)    
        else:
