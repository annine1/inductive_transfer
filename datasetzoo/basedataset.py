...
class BaseDataset(Dataset):
    ...
    def __init__(self,
        ...
        if basin is None:
            ...
            # self.basins1 = utils.load_basin_file(getattr(cfg, f"{period}_basin_file1"))    
        # elif basin1 is None:
        #     self.basins1 = utils.load_basin_file(getattr(cfg, f"{period}_basin_file1"))
        else:
            self.basins = [basin]
    ...
    def __getitem__(self, item: int) -> Dict[str, torch.Tensor]:
        ...
        for freq, seq_len, idx in zip(self.frequencies, self.seq_len, indices):
            ...
            # sample[f'y1{freq_suffix}'] = self._y1[basin][freq][idx - seq_len + 1:idx + 1]
            ...
            # if self._attributes1:
            #     static_inputs1.append(self._attributes1[basin])    
            if self._x_s:
                static_inputs.append(self._x_s[basin][freq][idx])
            # if self._x_s1:
            #     static_inputs1.append(self._x_s1[basin][freq][idx])    
            if static_inputs:
                sample[f'x_s{freq_suffix}'] = torch.cat(static_inputs, dim=-1)
            # if static_inputs1:
            #     sample[f'x_s1{freq_suffix}'] = torch.cat(static_inputs1, dim=-1)    
        ...
        for basin in tqdm(basin_coordinates, file=sys.stdout, disable=self._disable_pbar):
            ...
            with warnings.catch_warnings():
                ...
                flag = validate_samples(x_d=[x_d[freq] for freq in self.frequencies] if self.is_train else None,
                                        ...
                                        # x_s1=[x_s1[freq] for freq in self.frequencies] if self.is_train and x_s1 else None,
                                        y=[y[freq] for freq in self.frequencies] if self.is_train else None,
                                        # y1=[y1[freq] for freq in self.frequencies] if self.is_train else None,
    ...
    def _load_data(self):
        ...
        # print("xr bef", xr.variables["QObs(mm/d)"].values)
        # print("xr af", 0.5 * xr.variables["QObs(mm/d)"].values)
        
        # data = torch.cat((xr.variables[var].values))
#         for var in list_variable:
#             xr.variables[var].values = mixup_aug.call(xr.variables[var].values)
        ...
        # print("mean:", xr.mean())
        # print("std:", xr.std())
        # print(np.any(np.isnan(xr)))
        # print("xr af", xr.variables["QObs(mm/d)"])
        ...
        for j in prange(n_samples):  # iterate through lowest-frequency samples
            ...
            if y is not None:
                ...
                if np.prod(np.array(_y.shape)) > 0 and np.all(np.isnan(_y)):
                    ...
                    continue
            # if y1 is not None:
            #     _y1 = y1[i][last_sample_of_freq - predict_last_n[i] + 1:last_sample_of_freq + 1]
            #     if np.prod(np.array(_y1.shape)) > 0 and np.all(np.isnan(_y1)):
            #         flag[j] = 0
            #         continue    
            ...
            if x_s is not None:
                ...
                if np.any(np.isnan(_x_s)):
                    flag[j] = 0
            # if x_s1 is not None:
            #     _x_s1 = x_s1[i][last_sample_of_freq]
            #     if np.any(np.isnan(_x_s1)):
            #         flag[j] = 0        
